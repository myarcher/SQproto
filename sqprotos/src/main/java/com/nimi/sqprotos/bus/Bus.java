...
import com.nimi.sqprotos.bus.method.AnnotationMethodFinder;
import com.nimi.sqprotos.bus.method.MethodFinder;
import com.nimi.sqprotos.bus.method.NamedMethodFinder;
import com.nimi.sqprotos.bus.scheduler.Scheduler;
import com.nimi.sqprotos.bus.scheduler.Schedulers;
...
public class Bus {
     ...
     * @param <E>   事件类型
     */
    public <E> void postSticky(E event,int indexs) {
        post(event,indexs);
     ...
     * @param <E>   事件类型
     */
    public <E> void post(E event,int indexs) {
        ...
        if (mStrictMode) {
            postEventByTypeAndIndexs(event, theEventType,indexs);
            return;
        }
        ...
        for (Class<?> eventType : eventTypes) {
            postEventByTypeAndIndexs(event, eventType,indexs);
        }
        if (mDebug) {
     ...
     * @param <E>       事件类型
     */
    private synchronized <E> void postEventByTypeAndIndexs(final E event, final Class<?> eventType,int indexs) {
        final Set<Subscriber> subscribers = mSubscriberMap.get(eventType);
        if (subscribers == null || subscribers.isEmpty()) {
            return;
        }
        for (Subscriber subscriber : subscribers) {
            if (subscriber.indexs ==indexs){
                sendEvent(new EventEmitter(this, event, subscriber, mDebug));
        }
    }

...
public class StorageUtil {
    ...
    static File getApkFileDir() {
        return AppAppliction.applictions.getExternalFilesDir(DIR_NAME_APK);
    }
    ...
    static File getApkFile(String version) {
        String appName;
        try {
            appName = AppAppliction.applictions.getPackageManager()
                    .getPackageInfo(AppAppliction.applictions.getPackageName(), 0)
                    .applicationInfo.loadLabel(AppAppliction.applictions.getPackageManager())
            .toString();
        } catch (Exception e) {

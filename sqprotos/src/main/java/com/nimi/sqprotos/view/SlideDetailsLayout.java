...
public class SlideDetailsLayout extends ViewGroup {
    private final static String TAG = SlideDetailsLayout.class.getSimpleName();
    /**
     * 用于完成滚动操作的实例
     */
...
int mScreenHeight;
    ...
    * */
    ...
    public interface PositionChangListener{
        ...
        public void scrollY(int scrollY, int vi);
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        topView.layout(0,0,getMeasuredWidth(),topView.getMeasuredHeight());
      //  promptView.layout(0,getMeasuredHeight(),promptView.getMeasuredWidth(),getMeasuredHeight()+promptView.getMeasuredHeight());
        bottomView.layout(0,topView.getMeasuredHeight(),getMeasuredWidth(),topView.getMeasuredHeight()+bottomView.getMeasuredHeight());
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if(position == 0){
                    ...
                    if(positionChangListener!=null){
                        if(getScrollY() + bottomView.getTop() > bottomView.getTop()){
                            if(!bottom){
                    ...
                    if(positionChangListener!=null){
                        if(getScrollY() < bottomView.getTop()){
                            if(!top){
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(position == 0){
                    targetIndex = getScrollY() + topView.getBottom() >topView.getBottom() ? 1 : 0;

                }else{
                    targetIndex = getScrollY() < topView.getBottom() ? 0 : 1;
                ...
                int absScroll = getScrollY()+mScreenHeight-getChildAt(0).getMeasuredHeight();
                if(targetIndex == 0&&mScreenHeight/5<absScroll){
                    mScroller.startScroll(0, getScrollY(), 0, mScreenHeight-absScroll);
                }else if(targetIndex == 1&&mScreenHeight/5<absScroll){
                    mScroller.startScroll(0, getScrollY(), 0, -absScroll);
                }else {}
                ...
                if(getScrollY()!=0&&getScrollY()!=getHeight()){//如果处在滑动状态 就不要传递事件给子view
                    return true;
                }
    ...
    public void scrollToTop(){
        scrollTo(0, topView.getTop());
        if(positionChangListener!=null){
            positionChangListener.position(0);
        }
        postInvalidate();

    }
    public void scrollToBottom(){
        scrollTo(0, bottomView.getTop());
        if(positionChangListener!=null){
            positionChangListener.position(1);
        }
        postInvalidate();
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if(positionChangListener!=null){
            positionChangListener.scrollY(getScrollY(),topView.getBottom());
        }
    }
}
